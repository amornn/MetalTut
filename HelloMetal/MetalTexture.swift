...
class MetalTexture: NSObject {
  ...
  func loadTexture(device device: MTLDevice, commandQ: MTLCommandQueue, flip: Bool){
    ...
    let context = CGBitmapContextCreate(nil, width, height, bitsPerComponent, rowBytes, colorSpace, bitmapInfo)
  ...
  func image(mipLevel mipLevel: Int) -> UIImage{
    ...
    let colorSpace = CGColorSpaceCreateDeviceRGB()
    let bitmapInfo = CGImageAlphaInfo.PremultipliedLast.rawValue
    let context = CGBitmapContextCreate(p, mipmapedWidth, mipmapedHeight, 8, rowBytes, colorSpace, bitmapInfo)
